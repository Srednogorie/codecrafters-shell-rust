...
pub fn command_jobs_print(background_jobs: &mut Vec<BackgroundJob>, is_command_jobs: bool) {
        ...
        match job.child.try_wait() {
            Ok(None) => {
                if is_command_jobs {
                    if jobs_len == i {
                        println!(
                            "[{}]+  Running                 {} {} &", job.order_num, job.command, job.args.join(" ")
                        );
            ...
            Ok(Some(_)) => {
                if jobs_len == i {
                    println!("[{}]+  Done                 {} {}", job.order_num, job.command, job.args.join(" "));
                } else if jobs_len - 1 == i {
                    println!("[{}]-  Done                 {} {}", job.order_num, job.command, job.args.join(" "));
                } else {
                    println!("[{}]   Done                 {} {}", job.order_num, job.command, job.args.join(" "));
...
pub fn command_jobs(background_jobs: &mut Vec<BackgroundJob>) -> Result<(), std::io::Error> {
    command_jobs_print(background_jobs, true);
    Ok(())
}
